...
def create_stack(pair, output, ndv=-28672, compression='None',
    ...
    Stack will be formatted as:
        ...
        2           250m red from M[OY]D09GQ
        3           250m NIR from M[OY]D09GQ
        4           500m blue from M[OY]D09GA
        5           500m SWIR1 from M[OY]D09GA
        6           1000m Mask band from M[OY]D09GA
        7           1000m VZA * 100 from M[OY]D09GA
    ...
    out_ds = driver.Create(output,
                           ds_red.RasterYSize, ds_red.RasterXSize, 7,
                           gdal.GDT_Int16,
                           options=opts)
    ...
    out_ds.GetRasterBand(4).WriteArray(blue)
    ...
    out_ds.GetRasterBand(6).WriteArray(enlarge(mask, 4))

    vza = ds_vza.GetRasterBand(1).ReadAsArray().astype(np.int16)
    out_ds.GetRasterBand(7).WriteArray(enlarge(vza, 4))
